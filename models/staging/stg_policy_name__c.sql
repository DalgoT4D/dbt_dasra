with source as (
    ...
    select *
    from {{ source('salesforce_raw_data', 'Policy_Name__c') }}
...
cleaned as (
    ...
    select
        ...
        nullif(trim("Name"), '')               as policy_name,
...
-- deduplication logic to ensure we only have one record per Policy Name record
deduplicated as (
